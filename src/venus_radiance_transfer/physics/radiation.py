...
from ..config import HITRAN_CACHE_DIR, WN_START, WN_STOP, WN_STEP
...
def integrate_radiation(h_km, P_atm, T_K, n_all, gases, aerosol_modes):
    ...
    if gases is not None:
        for gas in gases:
            ...
            if not data_filename.exists():
                ...
                fetch(gas['name'], gas['M'], gas['I'], WN_START, WN_STOP)
            filename = _get_filename(gas['name'])
            if not filename.exists():
                print(f'Произвожу расчет коэффициентов поглощения газа {gas['name']}')
                compute_k_abs_gas_parallel(gas, n_layers, P_atm, T_aver, n_all, wn_grid)
    ...
    tau_above = np.zeros_like(wn_grid)
    I_up = np.zeros_like(wn_grid)
    
    # Цикл по слоям сверху вниз (чтобы накапливать tau_above)
    for i in range(n_layers-1, -1, -1):
        k_total = np.zeros_like(wn_grid)

        # 1. Газовые компоненты
        if gases is not None:
            for gas in gases:
                k_gas = load_k_abs_layer(gas['name'], i)
                k_total += k_gas

        # 2. Аэрозольная компоненты
        if aerosol_modes is not None:
            k_aer = load_k_abs_layer('aerosol', i)
            k_total += k_aer

        tau_i = k_total * dz_cm[i]
        
        # Яркостная температура слоя (можно взять среднюю между границами, но для простоты – на нижней)
        B = planck_intensity(wn_grid, T_K[i])
    ...
    # Добавляем излучение нижней границы (поверхность / облака), ослабленное всей атмосферой
    I0 = planck_intensity(wn_grid, T_K[0])
